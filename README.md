...
- Orientation Mode -- a single value the Component owns and publishes:
  - The effective Orientation Mode is the active override if one is set, otherwise the Movement Profile's default (`Movement.Orientation.OrientToControl` when unset). `OrientToControl` faces the Control Rotation (camera) and strafes; `OrientToMovement` faces the movement direction;
...
- Gameplay Tags (`Danzmann::GameplayTags` namespace): `Status.Sprinting`, `Movement.Orientation.{OrientToControl, OrientToMovement}`, and `Movement.Mode.{Walking, Falling, Flying, Swimming}`. The orientation and mode Tags are written exclusively by `UDanzmannMoverComponent` and are read-only for everyone else.
